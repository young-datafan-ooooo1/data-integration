...
public class ExcelWriterStep extends BaseStep implements StepInterface {
  ...
  private ExcelWriterStepMeta2 meta;

  private static Class<?> PKG = ExcelWriterStepMeta2.class; // for i18n
  ...
  public void prepareNextOutputFile() throws KettleException {
    try {
      ...
      data.file = KettleVFS.getFileObject( buildFilename, getTransMeta(), FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword()))  );
      ...
      if ( !data.file.exists() ) {
        ...
        if ( meta.isTemplateEnabled() ) {
          ...
          String templateExt = KettleVFS.getFileObject( data.realTemplateFileName ,FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(),DefaultEncryptionUtils.decrypt(meta.getFtpPassword())) ).getName().getExtension();
          ...
          if ( KettleVFS.getFileObject( data.realTemplateFileName, FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(),DefaultEncryptionUtils.decrypt(meta.getFtpPassword()))  ).exists() ) {
            // if the template exists just copy the template in place
            copyFile( KettleVFS.getFileObject( data.realTemplateFileName, getTransMeta(),FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(),DefaultEncryptionUtils.decrypt(meta.getFtpPassword())) ), data.file );
          } else {
  ...
  public boolean init( StepMetaInterface smi, StepDataInterface sdi ) {
    ...
    if ( super.init( smi, sdi ) ) {
      ...
      data.shiftExistingCells = ExcelWriterStepMeta2.ROW_WRITE_PUSH_DOWN.equals( meta.getRowWritingMethod() );
      data.createNewSheet = ExcelWriterStepMeta2.IF_SHEET_EXISTS_CREATE_NEW.equals( meta.getIfSheetExists() );
      data.createNewFile = ExcelWriterStepMeta2.IF_FILE_EXISTS_CREATE_NEW.equals( meta.getIfFileExists() );
      return true;
    }
    return false;
  }
