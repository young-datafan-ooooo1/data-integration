...
public class ExcelWriterStepMeta2 extends BaseStepMeta implements StepMetaInterface {
  private static Class<?> PKG = ExcelWriterStepMeta2.class; // for i18n purposes, needed by Translator2!!
  ...
  @Override
  public Object clone() {
    ExcelWriterStepMeta2 retval = (ExcelWriterStepMeta2) super.clone();
  ...
  public String exportResources( VariableSpace space, Map<String, ResourceDefinition> definitions,
    ...
    try {
      ...
      if ( !Utils.isEmpty( fileName ) ) {
        FileObject fileObject = KettleVFS.getFileObject( space.environmentSubstitute( fileName ), space , FileServerConfig
            .getFileSystemOptions(getFileServerType(),getFtpUsername(), DefaultEncryptionUtils
            .decrypt(getFtpPassword())));
        fileName = resourceNamingInterface.nameResource( fileObject, space, true );
      }
      if ( !Utils.isEmpty( templateFileName ) ) {
        FileObject fileObject = KettleVFS.getFileObject( space.environmentSubstitute( templateFileName ), space , FileServerConfig.getFileSystemOptions(getFileServerType(),getFtpUsername(),DefaultEncryptionUtils.decrypt(getFtpPassword())));
