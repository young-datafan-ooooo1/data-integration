...
public class ExcelOutput extends BaseStep implements StepInterface {
  ...
  public boolean openNewFile() {
    ...
    try {
      ...
      data.file = KettleVFS.getFileObject( data.realFilename, getTransMeta() , FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword())));
      if ( meta.isCreateParentFolder() ) {
      ...
      if ( !meta.isTemplateEnabled() ) {
        ...
        try ( FileObject templateFile = KettleVFS.getFileObject( templateFilename, getTransMeta(),FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(),DefaultEncryptionUtils.decrypt(meta.getFtpPassword())) ) ) {
