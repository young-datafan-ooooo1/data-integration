...
package org.pentaho.di.plugins.joinRows.step;
  ...
  public void initialize() throws KettleException {
    ...
    try {
      ...
      for ( int i = 1; i < rowSetsSize; i++ ) {
       ...
       FileObject fileObject = KettleVFS.getFileObject(directoryName, getTransMeta(), FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword())));

        if ( directoryName != null ) {

          file = new File(fileObject.getPublicURIString().substring(5));
