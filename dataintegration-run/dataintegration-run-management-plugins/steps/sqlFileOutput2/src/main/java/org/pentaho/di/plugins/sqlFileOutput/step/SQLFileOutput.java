...
public class SQLFileOutput extends BaseStep implements StepInterface {
  ...
  public String buildFilename() {
    return meta.buildFilename( environmentSubstitute( meta.getFileName()+ meta.getSqlNames() ), getCopy(), data.splitnr );
  }
    ...
    try {
      ...
      if ( meta.AddToResult() ) {
        ...
        ResultFile resultFile =
          new ResultFile(
            ResultFile.FILE_TYPE_GENERAL, KettleVFS.getFileObject(filename, getTransMeta(),FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword()))), getTransMeta().getName(), getStepname() );
      ...
      OutputStream fos = KettleVFS.getOutputStream( filename, getTransMeta(), FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword())),meta.isFileAppended() );
      outputStream = fos;
      ...
      try {
        ...
        if ( meta.isCreateParentFolder() ) {
          ...
          try {
            ...
            parentfolder = KettleVFS.getFileObject( filename, getTransMeta(),FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword())) ).getParent();
