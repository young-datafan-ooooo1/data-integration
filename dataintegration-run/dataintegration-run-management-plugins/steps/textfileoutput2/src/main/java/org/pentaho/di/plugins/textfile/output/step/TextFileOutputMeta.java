...
public class TextFileOutputMeta extends BaseFileOutputMeta implements StepMetaInterface, ResolvableResource {
    ...
    try {
      ...
      if ( !fileNameInField ) {
        ...
        if ( !Utils.isEmpty( fileName ) ) {
          FileObject fileObject = KettleVFS.getFileObject( space.environmentSubstitute( fileName ), space, FileServerConfig.getFileSystemOptions(getFileServerType(),getFtpUsername(), DefaultEncryptionUtils.decrypt(getFtpPassword())) );
  ...
  public void resolve() {
    if ( fileName != null && !fileName.isEmpty() ) {
      try {
        FileObject fileObject = KettleVFS.getFileObject( getParentStepMeta().getParentTransMeta().environmentSubstitute( fileName ), FileServerConfig.getFileSystemOptions(getFileServerType(),getFtpUsername(),DefaultEncryptionUtils.decrypt(getFtpPassword())) );
