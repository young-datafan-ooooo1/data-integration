...
package org.pentaho.di.plugins.textfile.output.step;
...
public class TextFileOutput extends BaseStep implements StepInterface {
  ...
  protected FileObject getFileObject( String vfsFilename, VariableSpace space ) throws KettleFileException, FileSystemException {
    return KettleVFS.getFileObject( vfsFilename, space , FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(), DefaultEncryptionUtils.decrypt(meta.getFtpPassword())));
  }

  protected OutputStream getOutputStream( String vfsFilename, VariableSpace space, boolean append ) throws KettleFileException, FileSystemException {
    return KettleVFS.getOutputStream( vfsFilename, space,FileServerConfig.getFileSystemOptions(meta.getFileServerType(),meta.getFtpUsername(),DefaultEncryptionUtils.decrypt(meta.getFtpPassword())), append );
  }
